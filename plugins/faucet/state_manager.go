...
func (s *StateManager) prepareMoreFundingOutputs() (err error) {
	...
	messagelayer.Tangle().LedgerState.UTXODAG.GetEvents().TransactionConfirmed.Attach(monitorTxConfirmation)
	defer messagelayer.Tangle().LedgerState.UTXODAG.GetEvents().TransactionConfirmed.Detach(monitorTxConfirmation)
