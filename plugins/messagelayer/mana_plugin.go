...
func configureEvents() {
	...
	Tangle().LedgerState.UTXODAG.GetEvents().TransactionConfirmed.Attach(onTransactionConfirmedClosure)
	...
	if err := daemon.BackgroundWorker("Mana", func(shutdownSignal <-chan struct{}) {
		...
		for {
			...
			case <-shutdownSignal:
				...
				Tangle().LedgerState.UTXODAG.GetEvents().TransactionConfirmed.Detach(onTransactionConfirmedClosure)
				storeManaVectors()
				shutdownStorages()
				return
			// case <-ticker.C:
